  ...
  return (
    <div style={styles.container}>
      <nav style={styles.nav}>
        <span style={styles.navTitle}>🗂️ TaskManager</span>
        <div style={styles.navRight}>
          <span style={styles.welcome}>Hi, {user.name} ({user.role})</span>
          {user.role === 'admin' && <Link to="/admin" style={styles.adminLink}>Admin Panel</Link>}
      ...
      <div style={styles.main}>
        ...
        <div style={styles.card}>
          <h3 style={styles.cardTitle}>{editId ? '✏️ Edit Task' : '➕ New Task'}</h3>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              {editId && <button style={styles.cancelBtn} type="button"
        ...
        <div style={styles.card}>
          ...
          {loading ? <p>Loading tasks...</p> : tasks.length === 0 ? <p style={styles.empty}>No tasks yet. Create one above!</p> :
            tasks.map(task => (
              <div key={task._id} style={styles.taskItem}>
                <div style={styles.taskTop}>
                  ...
                  <div style={styles.badges}>
                    ...
                    <span style={{ ...styles.badge, background: statusColor[task.status], cursor: 'pointer' }}
                      onClick={() => handleStatus(task)} title="Click to change status">{task.status}</span>
                  </div>
                </div>
...
const styles = {
  ...
  adminLink: { color: '#fde68a', fontWeight: '600', textDecoration: 'none', background: 'rgba(255,255,255,0.2)', padding: '0.3rem 0.8rem', borderRadius: '6px' },
  ...
  btn: { padding: '0.75rem 1.5rem', background: '#6366f1', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' },
  ...
  editBtn: { padding: '0.35rem 0.75rem', background: '#ede9fe', color: '#6366f1', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' },
  deleteBtn: { padding: '0.35rem 0.75rem', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' },
